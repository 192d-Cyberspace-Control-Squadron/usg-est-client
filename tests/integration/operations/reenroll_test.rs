...
use usg_est_client::{EstClient, EstClientConfig, EnrollmentResponse, ClientIdentity, csr::CsrBuilder};
...
async fn test_successful_reenrollment() {
    ...
    if result.is_err() {
        eprintln!("Re-enrollment test skipped due to fixture format: {:?}", result.err());
        return;
    }
